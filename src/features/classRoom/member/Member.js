...
function Member(props) {
    ...
    return (
      <GridToolbarContainer>
        ...
        {localStorage.getItem("isTeacher") === "true" && (
          <div
            className={styles.importMember}
            onClick={() => setImportMembers(!importMembers)}
          >
        ...
        {localStorage.getItem("isTeacher") === "true" && (
          <div
            className={
              removeMember.length > 0 ? styles.deleteMember : styles.none
            }
  ...
  return (
    <>
      <div className={styles.member}>
        <DataGrid
          ...
          onSelectionModelChange={(mail) => {
            ...
            const selectedRowData = member.filter((r) =>
              selectedEmail.has(r.email)
            );
      ...
      {importMembers && <UIAddMember onOpen={setImportMembers} />}
    </>
  );
}
