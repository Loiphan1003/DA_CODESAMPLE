...
import {
  stringAvatar,
  stringToBGColor,
} from "../../components/funC/genColorAvatar";
      ...
      const data = async () => {
        try {
          const response = isTeacher
            ? await PhongHocAPI.getByUidGiangVien(uId)
            : await PhongHocAPI.getByUid(uId);
        ...
        <NavLink to={`/room/${room.id}`} key={room.id} className={styles.room}>
          <Item sx={{ padding: "0px", width: "300px" }}>
            ...
            <Avatar
              ...
              {...stringAvatar(
                room.tenPhong.substr(0, room.tenPhong.length - 1)
              )}
            ></Avatar>
            <div className={styles.ownerName}>{room.tenHienThi}</div>
