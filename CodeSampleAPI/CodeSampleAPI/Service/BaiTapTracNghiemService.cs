...
namespace CodeSampleAPI.Service
{
    ...
    public class BaiTapTracNghiemService : IBaiTapTracNghiemService
    {
        ...
        {
            BaiTapTracNghiem btTN = _codeSampleContext.BaiTapTracNghiems.FirstOrDefault(p => p.IdbaiTapTracNghiem == id);
            if (btTN == null)
                return false;
            try
            {
        ...
        public List<CauHoi_SearchResult> searchBaiTapTN(string searchValue)
        {
            ...
            if (int.TryParse(searchValue, out id))
            {
                ...
                lsByID = (from bt in _codeSampleContext.BaiTapTracNghiems
                          where bt.IdbaiTapTracNghiem == id
                          select new CauHoi_SearchResult()
                          {
                              id = bt.IdbaiTapTracNghiem,
                              moTa = bt.CauHoi
                          }).ToList();
            }
            //fliter theo moTa
        ...
        {
            return _codeSampleContext.BaiTapTracNghiems.FirstOrDefault(p => p.IdbaiTapTracNghiem == id);
        }

        public bool editBTTN(BaiTapTracNghiem_Custom btTN_Custom)
        {
            BaiTapTracNghiem bt = new BaiTapTracNghiem();
            bt = _codeSampleContext.BaiTapTracNghiems.FirstOrDefault(p => p.IdbaiTapTracNghiem == btTN_Custom.id);
            if (bt != null)
            {
